...
/** 钉钉授权登入 */
// if(!$user->hasLogin()){
//     if(@strpos($_SERVER['HTTP_USER_AGENT'],'DingTalk')){
//         $response->redirect('/GmOauth?site=Dingtalk');
//     }else{
//         $response->redirect('/');
//     }
// }
...
list($prefixVersion, $suffixVersion) = strpos($options->version, '/') !== false
    ? explode('/', $options->version)
    : array($options->version, null);

$params = parse_url(isset($currentMenu[2])?$currentMenu[2]:'');
$adminFile = basename(isset($params['path'])?$params['path']:'');
