...
<section id="custom-field" class="typecho-post-option<?php if (empty($defaultFields) && empty($fields)): ?> fold<?php endif; ?>">
    ...
    <table class="typecho-list-table mono">
        ...
        <?php foreach ($fields as $field): ?>
            <tr>
                <td>
                    ...
                    <input type="text" name="fieldNames[]" value="<?php echo htmlspecialchars($field['name'] ?? ''); ?>" id="fieldname" class="text-s w-100 form-control">
                </td>
                <td>
        ...
        <tr>
            ...
            <td>
                <label for="fieldvalue" class="sr-only"><?php _e('字段值'); ?></label>
                <textarea name="fieldValues[]" placeholder="<?php _e('字段值'); ?>" id="fieldvalue" class="text-s w-100 form-control" rows="2"></textarea>
            </td>
            <td>
                <button type="button" class="btn btn-xs"><?php _e('删除'); ?></button>
            </td>
    ...
    <div class="description clearfix">
        <button type="button" class="btn btn-xs operate-add  btn-default" style="height: 35px; padding-top: 6px;"><?php _e('+添加字段'); ?></button>
